...
const searchProduct = tool(async ({ query, token }) => {
    ...
    const response = await axios.get(`http://shopping-app-ALB-911279738.eu-north-1.elb.amazonaws.com/api/products?q=${query}`, {
        headers: {
...
const addProductToCart = tool(async ({ productId, qty = 1, token }) => {
    ...
    const response = await axios.post(`http://shopping-app-ALB-911279738.eu-north-1.elb.amazonaws.com/api/cart/items`, {
        productId,
        qty
    }, {
        headers: {
