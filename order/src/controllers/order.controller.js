...
async function createOrder(req, res) {
    ...
    try {
        ...
        const cartResponse = await axios.get(`http://shopping-app-ALB-911279738.eu-north-1.elb.amazonaws.com/api/cart`, {
            headers: {
        ...
        const products = await Promise.all(cartResponse.data.cart.items.map(async (item) => {
            ...
            return (await axios.get(`http://shopping-app-ALB-911279738.eu-north-1.elb.amazonaws.com/api/products/${item.productId}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            })).data.data
